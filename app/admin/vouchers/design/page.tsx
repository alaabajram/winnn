...
export default async function VoucherDesignPage(props: any) {
  ...
  const [{ data: campaigns }, { data: settings }] = await Promise.all([
    sb.from("campaigns")
      .select("id,name,slug,draw_date,voucher_design,banner_url,sponsor_logo_url,campaign_prizes(position,title,value_cents,image_url),campaign_merchants(merchants(name,logo_url))")
